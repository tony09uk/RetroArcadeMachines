...
import { Component, ElementRef, EventEmitter, HostListener, OnInit, SimpleChanges } from '@angular/core';

import { SliderHandlerEnum } from '../enums/slider-handler.enum';
...
export class SliderComponent extends Utilities implements OnInit, OnChanges {
  ...
  public stepIndicatorPositions: number[] = []; // todo: is this correct?
  ...
  private initializeStepIndicator(): void {
  ...
  private updateCurrentValue(arrayValue: number[], privateChange: boolean = false): void {
  ...
  /*Method to set current selected values */
  private updateInitValues(arrayValue: number[]): void {
  ...
  private setModelValue(index: number, value: number): void {
    if (this.step > 1) {
  ...
  * on-panstart
  */
  public setHandlerActive(event: any, handlerIndex: number): void {
